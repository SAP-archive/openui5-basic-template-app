...
sap.ui.predefine('sap/ui/layout/library.support', ['./rules/Form.support'], function (FormSupport) {
    'use strict';
    return {
        name: 'sap.ui.layout',
...
sap.ui.predefine('sap/ui/layout/rules/Form.support', ['sap/ui/support/library'], function (SupportLib) {
    'use strict';
    ...
    function isSmartForm(oControl) {
        if (oControl && (oControl.isA('sap.ui.comp.smartform.SmartForm') || oControl.isA('sap.m.Panel') && oControl.getParent().isA('sap.ui.comp.smartform.SmartForm'))) {
            return true;
        }
        return false;
    }
    var oFormResponsiveLayoutRule = {
        ...
        check: function (oIssueManager, oCoreFacade, oScope) {
            oScope.getElementsByClassName('sap.ui.layout.form.Form').forEach(function (oForm) {
    ...
    var oFormTitleAndToolbarRule = {
        ...
        check: function (oIssueManager, oCoreFacade, oScope) {
            ...
            oScope.getElementsByClassName('sap.ui.layout.form.FormContainer').forEach(function (oFormContainer) {
                checkTitleAndToolbar(oFormContainer, 'FormContainer', oIssueManager);
            });
            oScope.getElementsByClassName('sap.ui.layout.form.SimpleForm').forEach(function (oSimpleForm) {
    ...
    var oFormTitleOrAriaLabelRule = {
        ...
        check: function (oIssueManager, oCoreFacade, oScope) {
            oScope.getElementsByClassName('sap.ui.layout.form.FormContainer').forEach(function (oFormContainer) {
                ...
                if (!oFormContainer.getTitle() && oFormContainer.getAriaLabelledBy().length == 0 && !oForm.getTitle() && oForm.getAriaLabelledBy().length == 0) {
                    oIssueManager.addIssue({
                        severity: Severity.High,
                        details: 'In ' + sName + ' ' + sId + ', FormContainer' + oFormContainer.getId() + ' has no Title assigned.',
                        context: { id: oFormContainer.getId() }
                    });
                }
            });
        }
    ...
    var oFormAllowedContentRule = {
        ...
        description: 'It is not allowed to use any layout as content of a Form or nest Forms' + ' This leads to issues with screen reader support, keyboard support and field alignment.' + '\nIt is also not supported to use other unsupported controls as content of the Form' + ' as it is not sure this controls will meet the alingment and intartaction requirements of the Form.',
        ...
        check: function (oIssueManager, oCoreFacade, oScope) {
            ...
            oScope.getElementsByClassName('sap.ui.comp.smartform.SmartForm').forEach(function (oSmartForm) {
    ...
    var oFormTitleInToolbarAriaRule = {
        ...
        description: 'If a Toolbar is used in a Form or FormContainer and the Toolbar has a Title inside' + ' it must be set to AriaLabelledBy to enable screen reader support',
    ...
    var oFormPropertiesOfOtherLayoutRule = {
        ...
        description: 'Some properties of SimpleForm are only valid for a special layout.' + ' If they are used with a different layout they have no effect.',
        ...
        check: function (oIssueManager, oCoreFacade, oScope) {
            oScope.getElementsByClassName('sap.ui.layout.form.SimpleForm').forEach(function (oSimpleForm) {
                ...
                if (sLayout != sap.ui.layout.form.SimpleFormLayout.ResponsiveGridLayout && sLayout != sap.ui.layout.form.SimpleFormLayout.ColumnLayout) {
                ...
                for (var i = 0; i < aFormContainers.length; i++) {
                    ...
                    for (var j = 0; j < aFormElements.length; j++) {
                        ...
                        for (var k = 0; k < aFields.length; k++) {
                            ...
                            if (bEditable) {
                                if (oField.isA('sap.ui.core.Icon') || oMetadata.hasProperty('displayOnly') && oField.getDisplayOnly()) {
                                    bError = true;
                                    break;
                                }
                            }
                            if (oField.isA('sap.m.InputBase') || oField.isA('sap.m.Select') || oField.isA('sap.m.CheckBox') || oField.isA('sap.m.RadioButton') || oField.isA('sap.m.RadioButtonGroup') || oField.isA('sap.m.Button') && oFormElement.getLabel() || oField.isA('sap.m.Slider') || oField.isA('sap.m.Switch') || oMetadata.hasProperty('displayOnly') && !oField.getDisplayOnly()) {
                                bEditableField = true;
                                if (!bEditable) {
                                    bError = true;
                                    break;
                        ...
                        for (var j = 0; j < aFormElements.length; j++) {
                            ...
                            if (!oLabel && oFormElement.getVisible()) {
                                ...
                                for (var k = 0; k < aFields.length; k++) {
                                    ...
                                    oMetadata = oField.getMetadata();
                                    if (oMetadata.getAssociation('ariaLabelledBy') && (!oField.getAriaLabelledBy() || oField.getAriaLabelledBy().length == 0) && !oField.isA('sap.m.Button') && !(oField.isA('sap.m.CheckBox') && oField.getText()) && !(oField.isA('sap.m.RadioButton') && oField.getText())) {
                                        oIssueManager.addIssue({
                                            severity: Severity.High,
